...
function AdminExploreSingapore() {
  ...
  const fetchProducts = async () => {
    try {
      const params = selectedLandmark ? { landmark_id: selectedLandmark } : {};
      const response = await axios.get(`${API}/explore-singapore-products`, { params });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProduct) {
        await axios.put(`${API}/admin/explore-singapore-products/${editingProduct.id}`, productData, { withCredentials: true });
        toast.success('Product updated successfully');
      } else {
        await axios.post(`${API}/admin/explore-singapore-products`, productData, { withCredentials: true });
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      await axios.delete(`${API}/admin/explore-singapore-products/${productId}`, { withCredentials: true });
  ...
  const handleEdit = (product) => {
    ...
    setFormData({
      ...
      landmark_id: product.landmark_id,
      stock: product.stock,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-playfair font-bold text-gray-900">Explore Singapore Products</h1>
        <button
      ...
      <div className="bg-white rounded-xl p-4 shadow-md mb-6">
        <select
          ...
          className="w-full md:w-64 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary font-inter"
        >
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider font-inter">Landmark</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {products.map((product) => {
              const landmark = landmarks.find(l => l.id === product.landmark_id);
              return (
                <tr key={product.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-inter">{landmark?.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2 font-inter">Landmark *</label>
                  <select
                    required
                    value={formData.landmark_id}
                    onChange={(e) => setFormData({...formData, landmark_id: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary font-inter"
                  >
...
export default AdminExploreSingapore;
