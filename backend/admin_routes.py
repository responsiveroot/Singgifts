...
async def upload_image(
    ...
    # Return the URL path (with /api prefix for Kubernetes routing)
...
async def create_product_admin(
    ...
    product_data = {
        ...
        "is_batik_label": product.get("is_batik_label", False),
        "is_on_deal": product.get("is_on_deal", False),
        "deal_percentage": float(product.get("deal_percentage", 0)) if product.get("deal_percentage") else None,
        "deal_start_date": product.get("deal_start_date"),
        "deal_end_date": product.get("deal_end_date"),
        "rating": 0,
        "review_count": 0,
